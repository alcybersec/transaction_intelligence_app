 ...
 *
 * When collapsed the trigger shows a compact preview ("3 included, 1
 * excluded"); the panel auto-expands on first render if any selections are
 * already present so users see what's set.
 */
export function CategoryPicker({ include, exclude, onChange }: Props) {
  const { data: cats = [] } = useCategories()
  const [open, setOpen] = useState(include.length + exclude.length > 0)

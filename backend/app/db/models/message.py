"""Message model for storing raw SMS and email evidence."""
...
class Message(Base):
    ...
    __table_args__ = (
        # Primary idempotency: same source + source_uid should not duplicate
        UniqueConstraint("source", "source_uid", name="uq_messages_source_uid"),
        # Secondary idempotency: same hash + observed_at window
        Index("ix_messages_hash_observed", "raw_body_hash", "observed_at"),
        # Index for finding messages by source and time

...
def test_email_normalized_on_patch(client, auth_headers):
    r = client.patch("/auth/me", headers=auth_headers, json={"email": "  Foo@Bar.COM "})
    assert r.status_code == 200
...
def test_email_must_be_unique(client, auth_headers, db_session, test_user):
    ...
    client.patch("/auth/me", headers=auth_headers, json={"email": "taken@example.com"})
    ...
    r = client.patch(
        "/auth/me", headers=u2_headers, json={"email": "TAKEN@example.com"}
    )  # different case
